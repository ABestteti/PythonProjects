...
x_axis = np.linspace( atributos_independentes[:, 0].min()-10 , atributos_independentes[:, 0].max()-2 , 6).round()
x_axis = list(map(str,list(map(int, x_axis))))
y_axis = np.linspace( atributos_independentes_treino[:, 1].min() , atributos_independentes_treino[:, 1].max() , 10).round()
